...
чем второе, просто поменяйте их местами. «step» по умолчанию равен = 0. Также не допускать ввод дробных чисел.
"""


def custom_ange(start, end, step=0):
    if start > end:
        start, end = end, start
    try:
        return (i for i in range(start, end + 1, step) if isinstance(start, end, step, int))
    except:
        print("Ошибка ввода, допускаются только целые числа, шаг отличный от '0'")
